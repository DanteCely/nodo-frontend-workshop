...
class State {
  constructor(grid, movida, tiempo, status) {
    this.grid = grid;
    this.movida = movida;
    this.tiempo = tiempo;
    this.status = status;
...
class Game {
  ...
  render() {
    const { grid, movida, tiempo, status } = this.state;
    ...
    if (status === "jugando") newButton.textContent = "Reset";
    ...
    // Render movida
    document.getElementById("movida").textContent = `Movida: ${movida}`;
